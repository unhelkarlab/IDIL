...
class AntMazeEnv_v0(_AntMazeEnv):
  ...
  def __init__(self):
    self.available_sub_goals = np.asarray(
        ((0., 0.), (16., 0.), (16., 16.), (0., 16.)), dtype=np.float32)
    self.current_goal = 0
  ...
  def reset(self):
    ...
    self.wrapped_env.set_xy(self.available_sub_goals[self.current_goal] +
  ...
  def _forward_reward(self, last_pos, current_pos):
    ...
    if self.current_goal in (
        0, 1, 2) and np.linalg.norm(current_pos[:2] - current_goal) < 4.:
      self.current_goal += 1
  ...
  def step(self, a):
    ...
    done = (not np.isfinite(current_pos).all() or current_pos[2] <= 0.3
            or current_pos[2] >= 1.1)
    ...
    return np.concatenate(
        (obs, self.available_sub_goals[-1])), (forward_reward +
                                               ctl_reward), (done
                                                             or reached), info
...
class AntMazeEnv_v1(_AntMazeEnv):
  ...
  def __init__(self):
    self.available_sub_goals = [
        np.asarray(((0., 0.), (16., 0.), (16., 16.), (0., 16.)),
                   dtype=np.float32),
        np.asarray(((0., 16.), (16., 16.), (16., 0.), (0., 0.)),
                   dtype=np.float32)
    ]
    self.current_goal = 0
    self.sub_task = 0
  ...
  def reset(self):
    ...
    self.current_goal = np.random.randint(0, 3)
    self.wrapped_env.set_xy(
        self.available_sub_goals[self.sub_task][self.current_goal] +
        np.random.rand(2) * 4. - 2.)
    return np.concatenate(
        (self._get_obs(), self.available_sub_goals[self.sub_task][-1]))
  ...
  def _forward_reward(self, last_pos, current_pos):
    ...
    reached = False
    rwd = 0.
    current_goal = self.available_sub_goals[self.sub_task][self.current_goal +
                                                           1]
    if self.current_goal in (
        0, 1, 2) and np.linalg.norm(current_pos[:2] - current_goal) < 4.:
      self.current_goal += 1
    if np.linalg.norm(current_pos[:2] -
                      self.available_sub_goals[self.sub_task][-1]) < 4.:
      self.current_goal = -1
      reached = True
      rwd += 100.
    last_d = np.linalg.norm(last_pos[:2] - current_goal)
  ...
  def step(self, a):
    ...
    info["reward_forward"] = forward_reward
    info["reward_ctrl"] = ctl_reward
    return np.concatenate((obs, self.available_sub_goals[self.sub_task][-1])), (
        forward_reward + ctl_reward), (done or reached), info
...
class AntPushEnv_v0(_AntMazeEnv):
  ...
  def __init__(self):
    self.current_goal = 0
    self.available_sub_goals = np.asarray(
        ((0., 0.), (-8., 0.), (-8., 8.), (0., 8.), (0., 19.)), dtype=np.float32)
  ...
  def _forward_reward(self, last_pos, current_pos):
    ...
    current_goal = self.available_sub_goals[self.current_goal + 1]
    if self.current_goal in (
        0, 1, 2) and np.linalg.norm(current_pos[:2] - current_goal) < 3.5:
      self.current_goal += 1
    if np.linalg.norm(current_pos[:2] - self.available_sub_goals[-1]) < 4.:
      self.current_goal = -1
      reached = True
...
class AntPushEnv_v1(_AntMazeEnv):
  ...
  def __init__(self):
    self.current_goal = 0
    self.available_sub_goals = [
        np.asarray(((0., 0.), (-8., 0.), (-8., 8.), (0., 8.), (0., 19.)),
                   dtype=np.float32),
        np.asarray(((-8., 8.), (-8., 0.), (0., 0.), (0., 8.), (11., 8.)),
                   dtype=np.float32)
    ]
    self.sub_task = 0
  ...
  def step(self, a):
    ...
    return np.concatenate(
        (obs,
         self.available_sub_goals[self.sub_task][-1])), (forward_reward +
                                                         ctl_reward), done, info
...
def run_environment(env_name, episode_length, num_episodes):
  ...
  for ep in range(num_episodes):
    ...
    for _ in range(episode_length):
      ...
      if done:
        break
    print('Episode %d reward: %.2f, Success: %d', ep + 1, rewards[-1],
          successes[-1])

  print('Average Reward over %d episodes: %.2f', num_episodes, np.mean(rewards))
  print('Average Success over %d episodes: %.2f', num_episodes,
        np.mean(successes))
