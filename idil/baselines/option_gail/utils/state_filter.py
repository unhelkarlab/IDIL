...
class StateFilter(object):
  ...
  def __call__(self, x, fixed=False):
    ...
    if self.mean is None or self.n_step < 1:
      if fixed:
        return x.clip(*self.clamp) if isinstance(x, np.ndarray) else x.clamp(
            *self.clamp)
      else:
        self.mean = x.copy()
        self.var = np.zeros_like(self.mean)
        self.n_step = 1
        return np.zeros_like(x) if isinstance(
            x, np.ndarray) else torch.zeros_like(x)
    else:
      ...
      return torch.as_tensor(fx, dtype=torch.float32, device=device).resize_(
          *size) if is_tensor else fx

  def state_dict(self):
    return {"mean": self.mean, "var": self.var, "n_step": self.n_step}
