...
class ARGConfig(Config):
  ...
  def parser(self, desc=""):
    ...
    for k in self._arg_dict:
      ...
      parser.add_argument(
          f"--{arg_name}",
          type=str,
          default=self._arg_dict[k]
          if isinstance(self._arg_dict[k], str) else repr(self._arg_dict[k]),
          help=help_msg)
  ...
  def _value_from_string(string: str, typeinst: type):
    if typeinst == str:
      ...
      raise TypeError("unknown type (str, tuple, list, int, float, bool), " +
                      f"but get {typeinst}")
