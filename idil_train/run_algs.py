...
def run_alg(config):
  ...
  if alg_name == "ogail":
    from idil_algs.baselines.option_gail.option_gail_learn import learn
    learn(config, log_dir, output_dir, path_iq_data, pretrain_name,
          eval_interval)
  elif alg_name == "idil_j":
    from idil_algs.IDIL_Joint.train import trainer
    trainer(config, path_iq_data, num_traj, log_dir, output_dir, log_interval,
            eval_interval)
  elif alg_name == "iql":
    from idil_algs.baselines.IQLearn.iql import run_iql
    run_iql(config, path_iq_data, num_traj, log_dir, output_dir, log_interval,
            eval_interval)
  elif alg_name == "sac":
    from idil_algs.baselines.IQLearn.iql import run_sac
    ...
    from idil_algs.IDIL.train import train
