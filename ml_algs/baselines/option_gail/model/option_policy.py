...
class MoEPolicy(torch.nn.Module):
  ...
  def __init__(self, config: DictConfig, dim_s=2, dim_a=2):
    ...
    self.dim_a = dim_a
    self.dim_c = config.dim_c
    self.policies = torch.nn.ModuleList(
        [Policy(config, dim_s, dim_a) for _ in range(self.dim_c)])
    self.device = torch.device(config.device)
  ...
  def policy_log_prob_entropy(self, st, ats):
    ...
    log_probs = (-(ats - means).square() / (2 * (logstds * 2).exp()) - logstds -
                 0.5 * math.log(2 * math.pi)).sum(dim=-1, keepdim=True)
    entropies = (0.5 + 0.5 * math.log(2 * math.pi) + logstds).sum(dim=-1,
                                                                  keepdim=True)
  ...
  def log_prob_action(self, st, ats):
    # N x C x 1
    means, logstds = self.a_mean_logstd(st)
    return (-((ats - means).square()) / (2 * (logstds * 2).exp()) - logstds -
            math.log(math.sqrt(2 * math.pi))).sum(dim=-1, keepdim=True)

  def sample_action(self, st, fixed=False):
    raw_action = torch.stack(
        [p.sample_action(st, fixed) for p in self.policies], dim=1)  # N x C x A
