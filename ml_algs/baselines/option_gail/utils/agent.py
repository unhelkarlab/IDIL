...
class _SamplerCommon(object):
  ...
  def filter_demo(self, sa_array):
    if self.state_filter.enable:
      return tuple((self.state_filter(s, fixed=True).to(self.device), a)
                   for s, a in sa_array)
    else:
      return sa_array
...
class _Sampler(_SamplerCommon):
  ...
  def __init__(self,
               ...
               n_thread=4,
               loop_func=None):
    super(_Sampler, self).__init__(seed,
                                   policy,
  ...
  def __del__(self):
    print(
        f"agent process is terminated, check if any subproc left: {self.pids}")
    for p in self.procs:
      p.terminate()

  @staticmethod
  def worker(seed: int, env, policy, state_filter, loop_func, state: Value,
             counter: Value, queue: _sQueue):
    ...
    while True:
      ...
      while state.value > 0:
        ...
        with counter.get_lock():
          if counter.value > 0:
            queue.send(
                (tuple(x.cpu() for x in traj), state_filter.state_dict()))
...
class _SamplerSS(_SamplerCommon):
  ...
  def __init__(self,
               ...
               n_thread=1,
               loop_func=None):
    super(_SamplerSS, self).__init__(seed,
                                     policy,
                                     use_state_filter=use_state_filter)
    if n_thread > 1:
      print("Warning: you are using single thread sampler,",
