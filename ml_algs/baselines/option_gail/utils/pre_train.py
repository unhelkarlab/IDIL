...
from .utils import validate, reward_validate
...
def policy_loss_option_v2(opolicy: OptionPolicy, sa_array, factor=1.):
  ...
  for s_array, a_array in sa_array:
    ...
    log_alpha, log_beta, log_trs, log_pis, entropy = opolicy.log_alpha_beta(
        s_array, a_array)
...
def policy_loss_option_v3(opolicy: OptionPolicy, sa_array, factor=1.):
  ...
  entropies = []
  for s_array, a_array in sa_array:
    log_alpha, log_beta, _, _, entropy = opolicy.log_alpha_beta(
        s_array, a_array)
    losses.append(-((log_alpha + log_beta).softmax(dim=-1) *
                    (log_alpha + log_beta)).mean())
...
def pretrain(policy: Union[OptionPolicy, Policy],
  ...
  for i in range(n_iter):
    optimizer.zero_grad()
    if is_option:
      loss = policy_loss_option_v3(policy,
                                   sa_array,
                                   factor=500. * math.exp(-i / 200.))
    else:
